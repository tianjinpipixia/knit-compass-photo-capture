...
`/capture/` は「1. 基本項目」を元の内容・配置のまま表示し、「2. 写真登録」で撮影や写真の追加を繰り返せます。素材01などの連番や、重複したメーカー選択欄は表示しません。

最下部に「保存・終了」と「保存・次の糸商」を並べます。前者は保存して入力を終了し、後者は保存成功後に糸商欄と写真が空の次の入力画面へ進みます。展示会は引き継ぎます。通常画面への切替は画面上部にあります。

既存フォームと保存処理を使用します。写真・素材のDB、Append Only保存、バックアップ形式は変更しません。保存失敗時は撮影中のフォームを維持し、次の撮影へ進みません。
