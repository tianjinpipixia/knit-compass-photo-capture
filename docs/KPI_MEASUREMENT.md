...
`usage-metrics.js` は、画面表示、検索操作、登録操作、バックアップ、書き出しなどの回数を日別・機能別に `localStorage kc_usage_metrics_v1` へ集計します。検索語、入力内容、氏名、写真、商品IDは保存しません。過去120日分だけを保持し、システム状態画面からKPIテンプレート互換CSVを書き出せます。

この自動集計は利用頻度を把握する補助KPIです。削減時間、正式登録件数、Human Review結果、根拠状態は自動推定せず、実測した値だけをKPI台帳へ追記します。
