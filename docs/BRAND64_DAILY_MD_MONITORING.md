...
MD分析は全64ブランドを同じ深さで処理しない。`config/brand-md-analysis-framework.json` を分析フレームの正本とし、10ブランドをTier Aとして発売時系列まで深掘りし、その他のBrand64は差分監視から重要シグナルが出た場合のみ一時的に深掘りする。
...
全64ブランドを毎回同じ深さで調べると精度と継続性が落ちるため、次の10ブランドをTier Aとして発売時系列まで追う。
