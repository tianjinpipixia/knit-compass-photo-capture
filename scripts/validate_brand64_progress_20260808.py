...
assert sum(r['official_current_verified'] == 'YES' for r in rows) == 64
batches = Counter(r['official_verification_batch'] for r in rows)
assert batches == {'2026-08-08-B1': 24, '2026-08-08-B2': 40}, batches

phases = Counter(r['research_phase'] for r in rows)
expected = {
    'OFFICIAL_BRAND_VERIFIED_PRODUCT_PENDING': 38,
...
print('brand64 progress: OK (64/64 official identity checks; 78 existing product rows)')
