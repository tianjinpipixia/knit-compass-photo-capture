...
def prompt(batch: List[Dict[str, Any]], date: str) -> str:
    lines = [
        ...
        "Return JSON only and preserve brand_id exactly.",
        "Brands:",
    ]
    for b in batch:
...
def write_missing(root: pathlib.Path, date: str, pointer: Dict[str, Any], count: int, reason: str) -> pathlib.Path:
    path = root / date[:4] / date[5:7] / f"brand64_gemini_primary_scan_{date}.json"
    artifact = {
...
def main() -> int:
    ...
    for index, batch in enumerate(chunks(brands, batch_size), start=1):
        ...
        try:
            ...
            accepted = 0
            for row in response.get("brands") or []:
            ...
            msg = f"{type(exc).__name__} batch {index}: {exc}"
            errors.append(msg); batch_logs.append({"batch_index": index, "error": msg})
    ...
    pointer = {
        **previous_pointer,
        "latest_attempted_scan_date": date,
        "latest_comparable_scan_date": date,
        "latest_comparable_artifact_path": str(path).replace("\\", "/"),
